...
# Function to run the visa scheduling code for a single user
def run_visa_scheduler(USERNAME, PASSWORD, SCHEDULE_ID, PRIOD_START, PRIOD_END, YOUR_EMBASSY):
    ...
    first_loop = True
    while 1:
        LOG_FILE_NAME = "log_"+ USERNAME + "_"  + str(datetime.now().date()) + ".txt"
        if first_loop:
            t0 = time.time()
            total_time = 0
            Req_count = 0
...
for index, row in df.iterrows():
    ...
    run_visa_scheduler(username, password, schedule_id, period_start, period_end, your_embassy)